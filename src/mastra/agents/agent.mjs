...
import { systemCommandTool } from '../tools/system-command-tool.mjs';
import { browserReadTool } from '../tools/browser-read-tool.mjs';
import { writeFileTool } from '../tools/write-file-tool.mjs';
import { weatherTool } from '../tools/weather-tool.mjs';
import { deepSearchTool } from '../tools/deepsearch.mjs';
import { webSearchTool } from '../tools/web-search-tool.mjs';
import { telegramSendTool, telegramSearchTool, telegramGetRecentTool, telegramGetUnreadTool } from '../tools/telegram-tool.mjs';
...
## Tools
\`webSearch\` — Search the internet. Use this for quick questions (news, facts, how-to, people, places, etc.)
...
\`deepSearch\` — Deep multi-step research on complex topics. Launches Oscar agent to search, read, cross-reference sources, and return a structured report with citations. Use this INSTEAD of webSearch when the topic is broad, complex, or needs deep investigation.
...
2. Quick fact → use \`webSearch\`. Complex/broad/deep topic → use \`deepSearch\` (launches Oscar for multi-step research)
